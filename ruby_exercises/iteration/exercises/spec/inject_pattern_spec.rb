RSpec.describe 'inject pattern test' do
  ...
  it 'test 7' do
    ...
    elements.each do |key, value|
      sum_of_second_values = sum_of_second_values + value
    end
  ...
  it 'test 8' do
    ...
    toppings.each do |topping, details|
      total_calories = total_calories + (details[:calories] * details[:quantity])
    end
  ...
  it 'test 9' do
    ...
    final_grade = 0
    grades.each do |category, grade_list|
      grade_average = (grade_list.sum.to_f / grade_list.length.to_f)
      final_grade = (final_grade + grade_average).to_f
    end
  ...
  it 'test 10' do
    ...
    menu.each do |item, details|
      ...
      if details[:flavors].size > 1
        flavors_string = flavors[0..-2].join(", ") + ", and " + flavors[-1]
      else
        flavors_string = flavors.join(", ")
      end

      if details[:gluten_free]
        gluten_free = "gluten free"
      else 
        gluten_free = "non gluten free"
      end

      printable_menu = printable_menu + "- #{flavors_string} #{item.to_s} (#{gluten_free})\n"
    end
